...
public class PlugVertexColorTools
{
    ...
    private static void ToColor(Mesh mesh)
    {
        ...
        for (var j = 0; j < mesh.vertexCount; j++)
        {
            ...
            //构造是按列，此处需要按行
            Matrix4x4 matrix = new Matrix4x4(new Vector3(mTangent.x, mTangent.y, mTangent.z).normalized,
                mBinormal.normalized, mNormal.normalized, Vector4.zero);
